...
void map_benchmark()
{
	...
	for (int i = 0; i < 420000; i++)
		m1[i] = i + 0.01;
	clock_gettime(CLOCK_MONOTONIC, &tp2);
	cout << "200000 []: " << tp2 - tp1 << "s\n";
	clock_gettime(CLOCK_MONOTONIC, &tp1);
	for (int i = 0; i < 200000; i++)
		m1.erase(i);
	clock_gettime(CLOCK_MONOTONIC, &tp2);
	cout << "200000 erase(): " << tp2 - tp1 << "s\n";
	clock_gettime(CLOCK_MONOTONIC, &tp1);
	for (int i = 0; i < 200000; i++)
		m1.insert(pair<const int, float>(i, i + 0.01));
	clock_gettime(CLOCK_MONOTONIC, &tp2);
	cout << "200000 insert(): " << tp2 - tp1 << "s\n";
	clock_gettime(CLOCK_MONOTONIC, &tp1);
	for (int i = 0; i < 200000; i++)
		m1.find(i);
	clock_gettime(CLOCK_MONOTONIC, &tp2);
	cout << "200000 find(): " << tp2 - tp1 << "s\n";
	clock_gettime(CLOCK_MONOTONIC, &tp1);
	for (int i = 0; i < 200000; i++)
		m1.count(i);
	clock_gettime(CLOCK_MONOTONIC, &tp2);
	cout << "200000 count(): " << tp2 - tp1 << "s\n";
	clock_gettime(CLOCK_MONOTONIC, &tp1);
	for (map<int, float>::iterator it = m1.begin(); it != m1.end(); it++);
	clock_gettime(CLOCK_MONOTONIC, &tp2);
	cout << "200000 iterations: " << tp2 - tp1 << "s\n";
	clock_gettime(CLOCK_MONOTONIC, &tp1);
	for (int i = 0; i < 5; i++)
		map<int, float> tmp(m1);
	clock_gettime(CLOCK_MONOTONIC, &tp2);
	cout << "5 * 200000 cpy ctr: " << tp2 - tp1 << "s\n";
	clock_gettime(CLOCK_MONOTONIC, &tp1);
	for (int i = 0; i < 2000; i++)
		m1.equal_range(i);
	clock_gettime(CLOCK_MONOTONIC, &tp2);
	...
	for (int i = 0; i < 5; i++)
		tmp = m1;
	clock_gettime(CLOCK_MONOTONIC, &tp2);
	cout << "5 * 200000 operator=: " << tp2 - tp1 << "s\n";
}
